...
static esp_err_t left_handler(httpd_req_t *req){
    WheelAct(turn_speed, LOW, LOW, turn_speed);
    delay(100);
    WheelAct(regular_speed, LOW, regular_speed, LOW);
    delay(15);
    WheelAct(LOW, LOW, LOW, LOW);
...
static esp_err_t turnRight_handler(httpd_req_t *req){
  ...
  Serial.println("Stop");
  delay(200);
  WheelAct(LOW, high_speed, high_speed, LOW);
...
void startCameraServer(){
   ...
   httpd_uri_t stream_uri = {
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        err = httpd_register_uri_handler(camera_httpd, &turnLeft_uri);
    if (err != ESP_OK) {
      Serial.printf("Failed to register handler for /turnLeft: %d\n", err);
    }

    err = httpd_register_uri_handler(camera_httpd, &turnRight_uri);
    if (err != ESP_OK) {
      Serial.printf("Failed to register handler for /turnRight: %d\n", err);
    }

    err = httpd_register_uri_handler(camera_httpd, &cross_uri);
    if (err != ESP_OK) {
