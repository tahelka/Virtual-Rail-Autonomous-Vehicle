...
stream_url = 'http://10.0.0.17:81/stream'
...
def setup_stream():
    global stream, bytes, stream_url
    stream = urllib.request.urlopen(stream_url)
    bytes = b''

def adjust_fps_based_on_queue_size(queue):
    global current_fps
    queue_size = queue.qsize()
    queue_capacity = queue.maxsize if queue.maxsize > 0 else 100  # Assuming a max queue size of 100 if not set
    queue_fill_percentage = (queue_size / queue_capacity) * 100

    if queue_fill_percentage > 70 and current_fps > 10:
        new_fps = max(current_fps - 10, 10)
        if new_fps != current_fps:
            set_fps(new_fps)
            setup_stream()
            print(f"Lowering FPS to {new_fps} due to queue fill percentage of {queue_fill_percentage:.2f}%")
    elif queue_fill_percentage < 20 and current_fps < 30:
        new_fps = min(current_fps + 10, 30)
        if new_fps != current_fps:
            set_fps(new_fps)
            setup_stream()
            print(f"Raising FPS to {new_fps} due to queue fill percentage of {queue_fill_percentage:.2f}%")
...
def receive_frames(stream, bytes, frame_queue):
    frame_counter = 0
    while True:
        jpg, bytes = get_stream_bytes(stream, bytes)
        frame = get_frame_from_bytes(jpg)
        if frame is not None and frame.size != 0:  # Check that frame is not None and not empty
            end_time = time.time()
            frame_queue.put(frame)
            # Make a copy of the frame
            frame_copy = frame.copy()
            # Save the frame copy with a filename that includes the timestamp
            filename = f"images/frame_{frame_counter}_{end_time}.jpg"
            cv2.imwrite(filename, frame_copy)
            frame_counter += 1
        
        # Adjust FPS based on queue size
        #adjust_fps_based_on_queue_size(frame_queue)
...
def main():
    ...
    setup_stream()  # Initial stream setup
    frame_queue = queue.Queue(maxsize=100)  # Set the maximum size of the queue
    #stream = urllib.request.urlopen(stream_url)
    #bytes = b''

    receive_thread = threading.Thread(target=receive_frames, args=(stream, bytes, frame_queue))
